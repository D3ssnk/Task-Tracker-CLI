...
def test_update(test_task_manager, test_json_file):
    ...
    with open(test_json_file, "r") as file:
        json_data = json.load(file)
    assert json_data == [{'id': 1, 'description': 'Make Water', "status": "todo"}, {'id': 2, 'description': 'Make Tea', "status": "in-progress"}, {'id': 3, 'description': 'Make Soup', "status": "done"}], "Ensure task was update and the rest remain the same"

    # test that when you update a task, the task gets updated, but consecutively
    test_task_manager.update(2, "Make Water")
    with open(test_json_file, "r") as file:
        json_data = json.load(file)
    assert json_data == [{'id': 1, 'description': 'Make Water', "status": "todo"}, {'id': 2, 'description': 'Make Water', "status": "in-progress"}, {'id': 3, 'description': 'Make Soup', "status": "done"}], "Ensure task was update and the rest remain the same"
